...
const R2_PREFIXES = ['art/', 'collections/', 'scripts/', 'tokens/', 'pages/', 'news/', 'thumb/', 'media/'];
/* ---- card thumbnails ----
   `thumb/{file}.webp` is the 192px WebP twin of `art/{file}` — what every
   card grid, search result and roster row draws instead of the 591px PNG
   (150 KB on average, 700 KB at the top; the thumbnail is 8 KB). It is a
   DERIVED file, and three rules keep it honest:
     - it may be missing, never wrong. /assets/thumb/ falls back to the
       original art in the same response (serveThumb), so a character with no
       thumbnail yet simply costs what it always did.
     - writing `art/{file}` DELETES `thumb/{file}.webp` (/api/upload and
       /api/bloodstar-art), so replaced art can never leave an old face on the
       cards. The editors upload a fresh one right after (assets/art-thumb.js);
       until they do, the fallback serves the new original.
     - who may write a thumbnail is exactly who may write its art:
       uploadSlotDenied() maps the key back before checking.
   The 1,000 committed art files have committed thumbnails beside them
   (assets/thumb/, generated by migration/optimize-images.js); R2 art gets
   its thumbnails from the editors and from the dashboard's backfill card. */
const MEDIA_WIDTHS = [320, 640, 1280];
function mediaSource(key) {
  const match = /^media\/(320|640|1280)\/((?:scripts|collections)\/[^/]+\.(?:png|jpe?g|webp))\.webp$/i.exec(key);
  return match && !key.includes('..') ? match[2] : '';
}
function cleanETag(value) { return String(value || '').replace(/^W\//, '').replace(/^"|"$/g, ''); }
async function mediaOriginalETag(env, origin, key) {
  const own = env.ART && await env.ART.head(key);
  if (own) return cleanETag(own.etag || own.httpEtag);
  const res = await env.ASSETS.fetch(new Request(origin + '/assets/' + key.split('/').map(encodeURIComponent).join('/'), { method: 'HEAD' }));
  return res.ok ? cleanETag(res.headers.get('ETag')) : '';
}

const THUMB_PREFIX = 'thumb/';
/* A thumbnail too small to hold a picture is not a thumbnail — it is a
   canvas that drew nothing, and it is worse than having none at all: the
   fallback above only fires when the file is ABSENT, so a blank one is
   served, and served happily, while the character's own page (which draws
   the full art) looks perfect. That is not hypothetical — one character on
   the wiki carried a 172-byte, 192x192, entirely transparent WebP and its
   card was an empty parchment tile from the moment it was uploaded, with
   nothing to say so. `onerror` never fires for it
   either: the file is a valid image, it simply has nothing in it.
   So a stored thumbnail under this many bytes is treated as absent, by
   serveThumb (falls through to the art) and by the dashboard's backfill
   scan (lists it, so a real one replaces it). The bar is set from what the
   wiki actually holds: the smallest genuine thumbnail of 2,063 is 1.8 KB
   and the blank one is 172 bytes, so anywhere in that gap is safe, and an
   icon plain enough to land under it costs only the original picture. */
const THUMB_MIN_BYTES = 512;
...
async function uploadSlotDenied(env, sess, key) {
  ...
  // the same owner, the same approved editors — so answer the question about
...
async function logActivity(env, sess, action, entityType, slug, name) {
  ...
  if (FEED_CHANGING_ACTIONS.has(action) || String(action || '').startsWith('bulk-')) {
    await bumpContentVersion(env, entityType);
  }
  let username = null;
  try {
...
// The announcement banner, memoised the same way: it was one D1 read per page
// view on every page of the site, for a setting that changes a few times a year.
let _announcementCache = null;
async function announcementValue(env) {
  if (_announcementCache && (Date.now() - _announcementCache.at) < SITE_TEXT_CACHE_MS) return _announcementCache.ann;
  let ann = null;
  try {
  ...
  ann = ann && ann.text ? ann : null;
  _announcementCache = { at: Date.now(), ann };
  return ann;
}
...
// `private`, and this is load-bearing: it keeps Cloudflare's OWN cache out of
// the way. With `public, max-age=0` Cloudflare stored the feed at the edge and
// revalidated it with this Worker on every request — and served the browser
// its stored copy under an ETag of its own making (a hash of the compressed
// body). The browser then sent THAT hash back as If-None-Match, which matched
// nothing here, so the 304 path below never fired for a real browser and every
// browse page re-downloaded the whole feed (400 KB compressed) on every visit.
// `private` makes Cloudflare pass the response straight through, the Worker's
// ETag reaches the browser intact, and a repeat visit costs one empty 304.
...
const FEED_CACHE_CONTROL = 'private, max-age=0, must-revalidate';
...
const FEED_DEPS = { characters: ['character', 'collection'], scripts: ['script'], collections: ['collection'] };
const SSR_DEPS = {
  c: ['character', 'collection', 'script'],
  s: ['script', 'character', 'collection', 'wikipage'],
  collection: ['collection', 'character', 'script', 'wikipage'],
  news: ['news', 'character', 'collection', 'script'],
  p: ['wikipage', 'character', 'script', 'collection']
};
function versionFor(snapshot, scopes) {
  if (!scopes) return snapshot.v;
  if (snapshot.failed) throw new Error('Content version unavailable');
  // Manual SQL writes that bump the legacy counter remain a full reset.
  const versions = snapshot.scopes;
  if (!versions || String(versions.global) !== snapshot.v) return 'legacy-' + snapshot.v;
  return [versions.all || 0, ...scopes.map(scope => versions[scope] || 0)].join('.');
}
async function contentVersion(env, scopes) {
  if (_contentVersionCache && Date.now() - _contentVersionCache.at < CONTENT_VERSION_CACHE_MS) {
    return versionFor(_contentVersionCache, scopes);
  }
  if (_contentVersionPending) return versionFor(await _contentVersionPending, scopes);
  const epoch = _contentVersionEpoch;
  const pending = (async () => {
    const snapshot = { at: Date.now(), v: '0', scopes: null };
    try {
      const r = await env.DB.prepare("SELECT value, (SELECT value FROM settings WHERE key='cache_versions') AS scopes FROM settings WHERE key=?")
        .bind(CONTENT_VERSION_KEY).first();
      if (r && r.value) snapshot.v = String(r.value);
      if (r && r.scopes) snapshot.scopes = JSON.parse(r.scopes);
      if (epoch === _contentVersionEpoch) _contentVersionCache = snapshot;
    } catch { snapshot.failed = true; /* retry; never match an old version-zero cache */ }
    return snapshot;
  })();
  _contentVersionPending = pending;
  try { return versionFor(await pending, scopes); }
...
async function bumpContentVersion(env, entityType) {
  const scope = ['character', 'collection', 'script', 'news', 'wikipage'].includes(entityType) ? entityType : 'all';
  try {
    await env.DB.prepare(`WITH old AS (
      SELECT CAST(COALESCE((SELECT value FROM settings WHERE key='content_version'),'0') AS INTEGER) AS v,
             (SELECT value FROM settings WHERE key='cache_versions') AS packed
    ), current AS (
      SELECT v, CASE WHEN json_valid(packed) AND CAST(json_extract(packed,'$.global') AS TEXT)=CAST(v AS TEXT)
        THEN packed ELSE json_object('all',v) END AS versions FROM old
    )
    INSERT INTO settings(key,value)
    SELECT 'content_version', CAST(v+1 AS TEXT) FROM current
    UNION ALL
    SELECT 'cache_versions', json_set(versions, '$.' || ?, COALESCE(json_extract(versions,'$.' || ?),0)+1, '$.global',v+1)
      FROM current WHERE true
    ON CONFLICT(key) DO UPDATE SET value=excluded.value`).bind(scope, scope).run();
  } catch { /* a failed invalidation must never undo an already-saved edit */ }
...
// the live corpus (1,831 published): the card feed is 2.2 MB / 413 KB
// compressed, and this is a third of it — night reminders, jinx text, quotes
// and custom JSON were 40% of what every browse page downloaded to draw 64px
// thumbnails. Every tool page (Script Builder, the editors, the Token Tool,
// Grimoire Forge, the importers) stays on `card`, whose exclude-list keeps the
// schema fields the exports need.
//
// The derived jinxCount keeps counts available without shipping jinx prose.
// Homepage-only lede/quote text now travels in /api/home for its featured card.
const GRID_FIELDS = [
  'slug', 'page', 'name', 'team', 'ability', 'creator', 'tags', 'appearsIn',
  'appearsInFrom', 'art', 'image', 'classification', 'curata', 'curataFrom',
  'status', 'v'
];
function gridRow(d) {
  const g = {};
  for (const k of GRID_FIELDS) if (d[k] !== undefined) g[k] = d[k];
  // `image` is the absolute URL twin of `art`; every card draws `art` first
  // and only falls back to `image` when there is none, so with an `art` the
  // twin is dead weight (5% of the feed).
  if (g.art && g.image !== undefined) delete g.image;
  if (Array.isArray(d.jinxes) && d.jinxes.length) g.jinxCount = d.jinxes.length;
  return g;
}

// Public display projections. Full/card endpoints retain their export schema.
const BROWSE_FIELDS = ['id', 'slug', 'displayName', 'name', 'author', 'description',
  'tagline', 'header', 'logo', 'characters', 'match', 'include', 'exclude',
  'classification', 'curata', 'curataFrom', 'v'];
function browseRow(d) {
  return Object.fromEntries(BROWSE_FIELDS.filter(k => d[k] !== undefined).map(k => [k, d[k]]));
}

/* ---- `v`: the row's version, for cache-busting image URLs ----
   `updated_at` (every save rewrites it) as base-36 seconds. The card and grid
   renderers append it to the row's art URL as `?v=…`, and the Worker serves a
   versioned image URL with a year-long immutable Cache-Control (see the
   /assets/ route) — so an unchanged icon costs a returning reader nothing at
   all, not even the conditional request the bare URL still makes. A new save
   is a new `v` is a new URL, which is how replaced art still shows at once.
...
// `card` drops character almanac fields; `grid` trims further for browsing.
// Script/collection `browse` projections omit editor/export-only fields.
// Full feeds and the character card feed keep the complete export schema.
async function buildPublicJSON(env, table, opts = {}) {
  const drafts = !!opts.includeDrafts;
  const chars = table === 'characters';
  // `grid` is the card feed with GRID_FIELDS applied on top — it is built
  // through the same trimming so the two can never disagree on what a card
  // is told about classification and Curata.
  const gridOnly = opts.fields === 'grid' && chars;
  const cardOnly = (opts.fields === 'card' || gridOnly) && chars;
...
async function cachedFeedBody(env, ctx, table, fields, knownVersion) {
  ...
  const key = `https://feed.internal/${table}.json?fields=${fields}&v=${version}&f=${FEED_FORMAT_V}`;
...
async function appearsInHref(env, ctx, value) {
  ...
  if (!wanted) return '';
  const version = await contentVersion(env, ['collection', 'script']);
  if (!_appearsInLinksCache || _appearsInLinksCache.version !== version) {
    const [collBody, scriptBody] = await Promise.all([
      cachedFeedBody(env, ctx, 'collections', 'full'),
      cachedFeedBody(env, ctx, 'scripts', 'full')
    ]);
    const links = new Map();
  ...
  const pending = (async () => {
    ...
    if (body === null) {
      const bodies = await Promise.all([
        cachedFeedBody(env, ctx, 'characters', 'card'),
        cachedFeedBody(env, ctx, 'collections', 'browse'),
        cachedFeedBody(env, ctx, 'scripts', 'browse')
      ]);
...
function versionAssetPaths(html) {
  return html.replace(/assets\/([a-z0-9._-]+\.(?:js|css))\b/gi,
  ...
  <header class="topbar">
    <div class="brand-group">
      <a class="brand" href="${R}">
        <img class="brand-skull" src="${R}assets/logo_skull.webp" alt="">
...
async function serveProfileShell(env, request, url) {
  ...
  return new Response(res.body, {
    status: res.status,
    headers: { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-cache, must-revalidate', Link: PAGE_LINK_HEADER }
  });
}
...
async function charsBySlug(env, slugs) {
  ...
  for (let i = 0; i < wanted.length; i += 90) {
    ...
    try {
      const { results } = await env.DB.prepare(
        `SELECT data, status, slug, url_slug, owner_id, updated_at FROM characters
      ...
      for (const r of results || []) {
        try {
          ...
          const v = rowVersion(r.updated_at);
          if (v) d.v = v; else delete d.v;
          // The uploading account's handle, for the ids this roster exports
          // under (Render.exportId). Derived from owner_id, never stored.
          const who = r.owner_id == null ? '' : owners[String(r.owner_id)];
          if (who) d.ownerName = who; else delete d.ownerName;
...
/* ---- image serving (/assets/art|thumb|collections|scripts|…) ----
   Three things, each of which used to cost a phone a round trip or a download
   it did not need:

   1. CONDITIONAL REQUESTS. Every image goes out `no-cache, must-revalidate`
      with the R2 object's ETag, so a returning browser asks "still this one?"
      — and the old code answered with the whole file every time (R2 was read
      unconditionally and the 200 went out as-is). The object is now read
      with `onlyIf: request.headers`, which makes R2 itself compare the
      If-None-Match and hand back the metadata with no body when it matches;
      that becomes an empty 304. A repeat page view no longer re-downloads
      thirty icons.

   2. VERSIONED URLS. A URL carrying `?v=` (the row's version — see
      rowVersion()) is served `public, max-age=31536000, immutable` and kept
      in the colo's edge cache (caches.default, keyed on the full URL), so an
      unchanged icon costs a returning reader nothing, not even the 304, and
      costs the Worker nothing past the cache lookup. Every card renderer
      appends the version; the bare URL keeps revalidating exactly as before,
      so nothing that links an image the old way changed behaviour, and
      replaced art still shows at once on both (a save is a new version).

   3. THUMBNAILS. /assets/thumb/{file}.webp is served from, in order: the R2
      thumbnail; the R2 ORIGINAL if there is one (art that was uploaded or
      replaced has no committed twin, and a committed thumbnail of the seed art
      would be stale); the committed thumbnail; the committed original. The
      page never sees a broken image and never has to know which case it hit.
      An original served AT the thumbnail URL is cached for an hour, not a
      year, so the real thumbnail takes over once it exists.

   The nosniff + sandbox CSP pair is defence in depth behind the upload
   whitelist: objects written before /api/upload checked types still carry
   whatever Content-Type was claimed at the time, and these make such an object
   inert. Access-Control-Allow-Origin lets the official script tool fetch()
   icons into its print exports — the same rule is in _headers for committed
   files. */
const IMAGE_CACHE_REVALIDATE = 'no-cache, must-revalidate';
const IMAGE_CACHE_IMMUTABLE = 'public, max-age=31536000, immutable';
// An original standing in for a missing thumbnail, at a versioned URL.
const IMAGE_CACHE_STANDIN = 'public, max-age=3600';

function imageHeaders(obj, key, cacheControl) {
  const headers = new Headers();
  if (obj) obj.writeHttpMetadata(headers);
...
// An R2 read that honours the browser's conditional headers. Returns null
// when there is no such object; a 304 when the browser's copy is current.
async function r2ImageResponse(env, key, request, cacheControl, conditional, minBytes) {
  const obj = await env.ART.get(key, conditional ? { onlyIf: request.headers } : undefined);
  if (!obj) return null;
  // Too small to be the picture it claims to be (see THUMB_MIN_BYTES): answer
  // as if it were not there, so the caller falls through to whatever stands in.
  if (minBytes && obj.size < minBytes) return null;
  const headers = imageHeaders(obj, key, cacheControl);
  // R2 answers a failed precondition (the ETag matched If-None-Match) with the
  // object's metadata and no body — exactly a 304.
  if (!('body' in obj) || !obj.body) return new Response(null, { status: 304, headers });
  return new Response(obj.body, { headers });
}

// A committed file (the assets binding) restamped for this route: this
// route's Cache-Control, and CORS in case _headers did not match the path.
// `conditional` forwards the browser's If-None-Match so the assets platform
// can answer 304 from its own ETag (only for the bare, revalidating URL — a
// versioned one is immutable and never asked about again).
async function committedImageResponse(env, origin, key, request, cacheControl, conditional) {
  const h = new Headers();
  const inm = request.headers.get('If-None-Match');
  if (inm && conditional) h.set('If-None-Match', inm);
  const res = await env.ASSETS.fetch(new Request(origin + '/assets/' + key.split('/').map(encodeURIComponent).join('/'), { headers: h }));
...
async function edgeImageHit(request) {
  try { return await caches.default.match(request); } catch { return null; }
}
function edgeImagePut(ctx, request, res) {
  if (!res || res.status !== 200) return;
  try {
    const put = caches.default.put(request, res.clone()).catch(() => {});
    if (ctx) ctx.waitUntil(put);
  } catch { /* the response still goes out */ }
}

async function serveR2Image(env, ctx, request, url, key) {
  const versioned = url.searchParams.has('v');
  if (versioned) {
    const hit = await edgeImageHit(request);
    if (hit) return hit;
  }
  const cc = versioned ? IMAGE_CACHE_IMMUTABLE : IMAGE_CACHE_REVALIDATE;
  let res = await r2ImageResponse(env, key, request, cc, !versioned);
  if (!res) {
    // Not in R2 -> committed static file (the bare URL keeps the assets
    // platform's own headers, so nothing changes for it).
    res = versioned
      ? await committedImageResponse(env, url.origin, key, request, cc, false)
      : await env.ASSETS.fetch(request);
  }
  if (!res) return env.ASSETS.fetch(request);
  if (versioned) edgeImagePut(ctx, request, res);
  return res;
}

async function serveMedia(env, ctx, request, url, key) {
  const source = mediaSource(key);
  if (!source) return new Response('Not found', { status: 404 });
  if (url.searchParams.has('v')) {
    const hit = await edgeImageHit(request); if (hit) return hit;
  }
  const originalETag = await mediaOriginalETag(env, url.origin, source);
  ...
  // Never cache an original at a derivative URL: an admin backfill should
  // take effect on the next visit without another save or a stale fallback.
  return (env.ART && await r2ImageResponse(env, source, request, IMAGE_CACHE_REVALIDATE, true)) ||
    await committedImageResponse(env, url.origin, source, request, IMAGE_CACHE_REVALIDATE, true) ||
    new Response('Not found', { status: 404 });
}

async function serveThumb(env, ctx, request, url, key) {
  const file = key.slice(THUMB_PREFIX.length);
  if (!/^[^/]+\.webp$/i.test(file) || file.includes('..')) {
    return new Response('Not found', { status: 404 });
  }
  const artKey = 'art/' + file.slice(0, -'.webp'.length);
  const versioned = url.searchParams.has('v');
  if (versioned) {
    const hit = await edgeImageHit(request);
    if (hit) return hit;
  }
  const own = versioned ? IMAGE_CACHE_IMMUTABLE : IMAGE_CACHE_REVALIDATE;
  const standIn = versioned ? IMAGE_CACHE_STANDIN : IMAGE_CACHE_REVALIDATE;
  let res = null;
  if (env.ART) {
    res = await r2ImageResponse(env, key, request, own, !versioned, THUMB_MIN_BYTES);
    if (!res) res = await r2ImageResponse(env, artKey, request, standIn, !versioned);
  }
  if (!res) res = await committedImageResponse(env, url.origin, key, request, own, !versioned);
  if (!res) res = await committedImageResponse(env, url.origin, artKey, request, standIn, !versioned);
  if (!res) return new Response('Not found', { status: 404 });
  if (versioned) edgeImagePut(ctx, request, res);
  return res;
}

// Writing `art/{file}` retires its thumbnail (see THUMB_PREFIX). Fire and
// forget: a delete that fails leaves a stale thumbnail for one reader, an
// upload that waited on it would fail the save.
function dropThumbFor(env, ctx, key) {
  if (!env.ART || !key.startsWith('art/')) return;
  try {
...
/* Public reading HTML is shared by every viewer. Only cookie-free GET
   ...
   fallback renders never enter this cache. Browser responses remain no-store.
   Cache hits still count views and strip the internal marker.
   Dependency versions govern content freshness (5 s memo); BUILD_ID also
...
async function ssrRoute(env, ctx, request, url, build) {
  ...
  const publicPage = res.status === 200 && res.headers.has(VIEW_HEADER) &&
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (method === 'GET' && (path === '/characters.json' || path === '/collections.json' || path === '/scripts.json')) {
      ...
      const wantFields = url.searchParams.get('fields');
      const fields = table === 'characters'
        ? (wantFields === 'card' ? 'card' : wantFields === 'grid' ? 'grid' : 'full')
        : (wantFields === 'browse' ? 'browse' : 'full');
      ...
      const version = await contentVersion(env, FEED_DEPS[table]);
      const etag = `W/"${table}-${fields}-v${version}-f${FEED_FORMAT_V}"`;
    ...
    if (method === 'GET' && path === '/api/page-viewer') {
      const sess = await getSession(env, request);
      if (!sess) return jsonResponse({});
      const flags = await getAccountFlags(env, sess.userId);
      if (!flags || flags.banned) return jsonResponse({});
      sess.isAdmin = !!flags.is_admin;
      const type = url.searchParams.get('type'), slug = url.searchParams.get('slug') || '';
      if (!['character', 'script', 'collection'].includes(type) || !slug || slug.length > 180) return jsonResponse({});
      const row = type === 'collection' ? await findCollectionRow(env, slug)
        : await env.DB.prepare(`SELECT * FROM ${type === 'character' ? 'characters' : 'scripts'} WHERE slug=?`).bind(slug).first();
      if (!row || row.status !== 'published') return jsonResponse({});
      const mayOwn = canEditRow(sess, row), mayEdit = mayOwn || await canEditPage(env, sess, type, row);
      const d = foldLegacyCurata(parseData(row)); d.slug = row.slug;
      if (type === 'character') return jsonResponse({
        notice: mayEdit && Classify.isIncomplete(d) ? partialNoticeHTML(d, '/').replace(PARTIAL_PREHIDE, '') : ''
      });
    ...
    if (method === 'GET' && path === '/api/announcement') {
      return jsonResponse({ announcement: await announcementValue(env) });
    }

    // ---------- BOOT: everything site.js asks for on EVERY page, in one call ----------
    // The system-text overrides and the announcement are both public, both
    // tiny, both identical for every reader, and site.js used to fetch them as
    // two requests on every page load — two Worker invocations and, on a
    // phone, two round trips before the banner could show. One response now.
    ...
    // ---------- ADMIN: CARD THUMBNAILS ----------
    // Which characters still draw their card from the full-size art because
    // no thumb/{file}.webp exists for it (see THUMB_PREFIX). "Missing" means:
    // no usable thumbnail in R2 — none at all, or one under THUMB_MIN_BYTES,
    // which is a canvas that drew nothing and which serveThumb ignores for
    // exactly that reason — AND either the art itself is in R2 (uploaded or
    // replaced — a committed thumbnail of the seed art would be stale, so
    // serveThumb ignores it) or there is no committed thumbnail either. The
    // dashboard's "Card thumbnails" card feeds this list to
    // ArtThumb.uploadFromUrl(), one character at a time, in the admin's own
    // browser — the Worker cannot resize an image, a browser can.
    if (method === 'GET' && path === '/api/admin/thumb-missing') {
      const sess = await adminSession(env, request);
      if (!sess) return jsonResponse({ error: 'Not authorized' }, { status: 403 });
      if (!env.ART) return jsonResponse({ error: 'Image storage (R2) is not configured' }, { status: 500 });
      // key -> byte size: a thumbnail is judged on its size as well as its
      // existence, and `size` is already on every object the listing returns.
      const listAll = async prefix => {
        const keys = new Map(); let cursor;
        do {
          const page = await env.ART.list({ prefix, cursor, limit: 1000 });
          for (const o of page.objects || []) keys.set(o.key, o.size);
          cursor = page.truncated ? page.cursor : undefined;
        } while (cursor);
        return keys;
      };
      const [r2Thumbs, r2Art] = await Promise.all([listAll(THUMB_PREFIX), listAll('art/')]);
      let committed = new Set();
      try {
        const m = await env.ASSETS.fetch(new Request(url.origin + '/assets/thumb/manifest.json'));
        if (m.ok) committed = new Set(((await m.json()).files || []).map(f => THUMB_PREFIX + f));
      } catch { /* no committed thumbnails */ }
      const { results } = await env.DB.prepare(
        "SELECT slug, name, data FROM characters WHERE status IN ('published','draft')"
      ).all();
      const missing = []; let withArt = 0, blank = 0;
      for (const r of results || []) {
        let art = '';
        try { art = String(JSON.parse(r.data).art || ''); } catch { continue; }
        if (!/^art\/[^/]+\.(png|jpe?g|webp|gif)$/i.test(art)) continue;
        withArt++;
        const thumbKey = THUMB_PREFIX + art.slice(4) + '.webp';
        const size = r2Thumbs.get(thumbKey);
        if (size !== undefined && size >= THUMB_MIN_BYTES) continue;
        // A blank thumbnail is listed whatever else exists: it sits in R2, so
        // it shadows both the art and any committed thumbnail, and the only
        // way out is a real one written over it.
        const isBlank = size !== undefined;
        if (!isBlank && !r2Art.has(art) && committed.has(thumbKey)) continue;
        if (isBlank) blank++;
      ...
      return jsonResponse({
        ok: true, withArt, r2Thumbs: r2Thumbs.size, committed: committed.size,
        missing: missing.length, blank, items: missing.slice(0, 2500)
      });
    }
    ...
    if (method === 'POST' && path.startsWith('/api/')) {
      ...
      if (path === '/api/upload') {
        ...
        const stored = await env.ART.put(key, bytes, {
          httpMetadata: { contentType },
          customMetadata: { owner: String(sess.userId), ...(mediaKey ? { sourceETag } : {}) }
        });
