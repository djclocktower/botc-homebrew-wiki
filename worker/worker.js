...
async function r2ImageResponse(env, key, request, cacheControl, conditional) {
...
async function serveThumb(env, ctx, request, url, key) {
  ...
  let res = null;
  if (env.ART) {
    res = await r2ImageResponse(env, key, request, own, !versioned);
...
export default {
  async fetch(request, env, ctx) {
    ...
    // no thumbnail in R2, AND either the art itself is in R2 (uploaded or
    ...
    if (method === 'GET' && path === '/api/admin/thumb-missing') {
      ...
      for (const r of results || []) {
        ...
        if (r2Thumbs.has(thumbKey)) continue;
        if (!r2Art.has(art) && committed.has(thumbKey)) continue;
        missing.push({ slug: r.slug, name: r.name, art });
      }
      return jsonResponse({
        ok: true, withArt, r2Thumbs: r2Thumbs.size, committed: committed.size,
        missing: missing.length, items: missing.slice(0, 2500)
      });
    }
