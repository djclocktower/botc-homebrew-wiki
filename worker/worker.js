...
const R2_PREFIXES = ['art/', 'collections/', 'scripts/', 'tokens/', 'pages/', 'news/', 'thumb/'];
/* ---- card thumbnails ----
...
//
// Two derived stamps ride along: `jinxCount` (the homepage counts jinxed
// characters on a collection tile — the count is a number, the list was 6% of
// the feed) and `quote` only when there is no `lede` (Featured falls back
// through lede → quote; the pages with both were paying for both).
const GRID_FIELDS = [
...
function gridRow(d) {
  ...
  if (!g.lede && d.quote) g.quote = d.quote;
...
async function buildPublicJSON(env, table, opts = {}) {
  ...
  return gridOnly ? out.map(gridRow) : out;
}
...
async function committedImageResponse(env, origin, key, request, cacheControl, conditional) {
  ...
  const res = await env.ASSETS.fetch(new Request(origin + '/assets/' + key, { headers: h }));
...
/* ---- the server-rendered pages: one response builder, one edge cache ----
   Every SSR page (/c/, /s/, /collection/, /news/, /p/) goes out through
   htmlPage(): `no-store` for the browser — the page carries whatever the
   viewer's login shows them, and a back-button must never replay a stale
   draft bar — plus the same Link preload header _headers gives the static
   pages, so the stylesheet and the two above-the-fold faces start downloading
   while the HTML is still arriving (and Cloudflare replays them as a 103
   Early Hints response once that is switched on for the zone).

   ssrRoute() is the cache. For a reader with NO session cookie — which is
   most readers, and every reader arriving from a Discord link — a published
   page is identical for everybody: no edit button, no draft bar, no Partial
   notice (all of those are decided from the session, and the client-side
   widgets ask /api/me and /api/comments themselves). So that HTML is kept in
   caches.default under a key carrying the content version, the origin and
   the path, and served from there on the next request without touching D1:
   a /c/ page used to cost 6–10 queries and ~1.4 s of TTFB, and the cached
   copy costs the version lookup (5 s memo). Invalidation is the version —
   every content write bumps it and rolls every key — exactly like the feeds.

   What is and is not stored:
     - only 200 HTML, only for a cookie-less GET, and only a response the
       route marked with X-Botc-View — which it does ONLY for a published
       page. A draft never reaches a cookie-less reader (the routes 404 it),
       a committed static page falling through assetsOrNotFound carries no
       mark, redirects and 404s are not 200.
     - X-Botc-View is `type|slug`, the page's view-counter key. A cache hit
       skips the route, so ssrRoute counts the view itself from that header
       and strips it before the response leaves. Views therefore keep
       counting exactly as before; only the rendering is skipped.
...
async function ssrRoute(env, ctx, request, url, build) {
  if (request.method !== 'GET' || hasSessionCookie(request)) {
    return stripViewHeader((await build()) || await assetsOrNotFound(env, request));
  }
  let cacheReq = null;
  try {
    const version = await contentVersion(env);
    ...
    if (hit) {
      ...
      if (bar > 0 && ctx) ctx.waitUntil(bumpView(env, request, view.slice(0, bar), view.slice(bar + 1)));
      return stripViewHeader(new Response(hit.body, { status: 200, headers: hit.headers }));
    }
  } catch { cacheReq = null; /* the cache is an optimisation, never a dependency */ }
  const res = (await build()) || await assetsOrNotFound(env, request);
  if (cacheReq && res.status === 200 && res.headers.has(VIEW_HEADER) &&
      (res.headers.get('Content-Type') || '').startsWith('text/html')) {
    try {
      const headers = new Headers(res.headers);
      headers.set('Cache-Control', SSR_EDGE_CACHE_CONTROL);
      const stored = new Response(res.clone().body, { status: 200, headers });
      const put = caches.default.put(cacheReq, stored).catch(() => {});
      if (ctx) ctx.waitUntil(put);
    } catch { /* uncached, still served */ }
  }
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (method === 'GET' && (path === '/characters.json' || path === '/collections.json' || path === '/scripts.json')) {
      ...
      const fields = wantFields === 'card' ? 'card' : wantFields === 'grid' ? 'grid' : 'full';
    ...
    if (method === 'GET' && path.startsWith('/news/')) {
      return ssrRoute(env, ctx, request, url, async () => {
